...
口座ごとの初期残高と、計画開始月以降のフロー（収入・支出・振替・イベント）を登録することで、現在から未来までの資産推移をシミュレーションできる。計算はすべてブラウザ内で完結し、データはローカルに保持する。

## 設計原則

以下は「特徴」でもあり、実装上の制約でもある。

- **現在から未来を扱う**: 計画開始月時点の残高を口座ごとの初期残高として持ち、以降をフローで動かす。過去の月別断面は扱わない。
